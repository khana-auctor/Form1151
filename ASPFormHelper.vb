...
Public Class ASPFormHelper
    ...
    Public Function ExecuteCommonStartupTasks() As Boolean
        ...
        txtActualUser = HttpContext.Current.Session("txtActualUser")
        ...
        If Not String.IsNullOrEmpty(HttpContext.Current.Request.QueryString("taskID")) Or Not String.IsNullOrEmpty(ConfigurationManager.AppSettings("TestTaskID")) Then
            ...
            If Not String.IsNullOrEmpty(HttpContext.Current.Request.QueryString("taskID")) Then
                HttpContext.Current.Session("taskID") = HttpContext.Current.Request.QueryString("taskID")
            Else
                HttpContext.Current.Session("taskID") = ConfigurationManager.AppSettings("TestTaskID")
            End If
            ...
            Dim oTask As deTask = oRuntime.getTask(HttpContext.Current.Session("taskID"))
            Dim oInst As deWorkflowInstance = oTask.getInstance()

            RefID = oInst.instanceName
            System.Web.HttpContext.Current.Session("FormID") = RefID
            '  TestUser = oDesign.getUser(oTask.taskOwnerID).externalUserID
            ' AssignedToUserId = oDesign.getUser(oTask.taskAssignedTo).externalUserID
            'Changed because the validation wasn't working correctly.
            ...
            HttpContext.Current.Session("FormXML") = myStr
            ...
            doc = New XmlDocument
            doc.LoadXml(HttpContext.Current.Session("FormXML"))
        ...
        If Not String.IsNullOrEmpty(HttpContext.Current.Session("taskID")) Then
        ...
        If Not String.IsNullOrEmpty(HttpContext.Current.Session("taskID")) Then
            ...
            End If
            System.Web.HttpContext.Current.Session("xmlFileName") = myXMLFiles & System.Web.HttpContext.Current.Session("FormID") & ".xml"
        Else 'or process a new worklist item
            File.AppendAllText(myLog, "Starting New Form " & Now() & vbCrLf)
            Originator = txtActualUser
            CurrentTaskName = WorkflowStepNames.CompleteForm
            Dim myMiscWS As New Miscellaneous.Miscellaneous
            System.Web.HttpContext.Current.Session("FormID") = GetNextRefNumber()
            FormXML.LoadXml(My.Resources.sampledata.ToString)
            isFormNew = "true"
            HttpContext.Current.Session("ReviewActions") = _KDOTFormClass.DefaultActions
            HttpContext.Current.Session("FormXML") = FormXML.InnerXml
            System.Web.HttpContext.Current.Session("xmlFileName") = myXMLFiles & System.Web.HttpContext.Current.Session("FormID") & "_00.xml"
            UpdateXMLDocument("Folio", System.Web.HttpContext.Current.Session("FormID"))
            UpdateXMLDocument("FormId", System.Web.HttpContext.Current.Session("FormID"))
        ...
        If Not String.IsNullOrEmpty(HttpContext.Current.Session("taskID")) Then
            If Not CheckItemStatus(HttpContext.Current.Session("taskID"), AllocatedUser) Then
                Return False
            End If
        Else
            CheckItemStatus(HttpContext.Current.Session("taskID"), EffectiveUserid)
        End If
    ...
    Public Sub ReleaseCurrentTask()
        ...
        myws3.ReleaseWI2(System.Web.HttpContext.Current.Session("FormID"), HttpContext.Current.Session("taskID"))
        HttpContext.Current.Session.Abandon()
    End Sub

    Public Function isWorkItemLocked() As Boolean
        Dim myWS As New Maintenance.Maintenance
        Dim result As String = myWS.isWorkItemLocked(HttpContext.Current.Session("taskID"))
    ...
    Public Function CheckItemStatus(ByVal SN As String, ByVal AllocatedUser As String) As Boolean
        Dim myWS As New Maintenance.Maintenance
        Dim result As String = myWS.UpdateSessionStatus(System.Web.HttpContext.Current.Session("taskID"), Session_ID, System.Web.HttpContext.Current.Session("FormID"), isFormNew, AllocatedUser, txtActualUser, _KDOTFormClass.FormName)
    ...
    Public Sub LoadAttachments()
        ...
        attachlist = myWs.GetAttachmentsfromSMOBox(System.Web.HttpContext.Current.Session("FormID"))
    ...
    Public Sub SetPageFields()
        ...
        FormXML.InnerXml = HttpContext.Current.Session("FormXML")
    ...
    Public Sub ProcessWFItem(ByVal xmlDoc As XmlDocument)
        ...
        Try
            ...
            '  WFParms.Add("Area", GetXMLValue("AreaNum"))
            ' WFParms.Add("District", GetXMLValue("DistrictNum"))
            'WFParms.Add("ProcessFolio", GetXMLValue("FormId"))
            WFParms.Add("ReviewAction", GetXMLValue("ReviewAction"))
            ' WFParms.Add("SubArea", GetXMLValue("SubareaNum"))
            'WFParms.Add("Supervisor", GetXMLValue("Supervisor"))
            WFParms.Add("TestMode", GetXMLValue("TestMode"))
            WFParms.Add("UpdatedXML", HttpContext.Current.Session("FormXML").ToString)
            '  K2Proc.DataFields("CurrentTaskName").Value = GetXMLValue("CurrentTaskName")
            '  K2Proc.DataFields("EffectiveUserid").Value = GetXMLValue("EffectiveUserId")
            '  K2Proc.DataFields("FormOriginator").Value = GetXMLValue("txtActualUser")
    ...
    Public Sub StartingNewWF(ByVal XMLDoc As XmlDocument)
        Try
            ...
            '  WFParms.Add("Area", GetXMLValue("AreaNum"))
            ' WFParms.Add("District", GetXMLValue("DistrictNum"))
            WFParms.Add("ProcessFolio", System.Web.HttpContext.Current.Session("FormID"))
            WFParms.Add("ReviewAction", GetXMLValue("ReviewAction"))
            ' WFParms.Add("SubArea", GetXMLValue("SubareaNum"))
            'WFParms.Add("Supervisor", GetXMLValue("Supervisor"))
            WFParms.Add("TestMode", GetXMLValue("TestMode"))
            ...
            Dim oInst As deWorkflowInstance = oDef.createInstance(System.Web.HttpContext.Current.Session("FormID"))
        ...
        Catch ex As Exception
            Dim myMSG As String = "Error StartingNewWF: " & ex.Message
    ...
    Public Sub DoSubmit()
        File.AppendAllText(myLog, "Starting DoSubmit in ASPFormhelper FormID: " & System.Web.HttpContext.Current.Session("FormID") & Now() & vbCrLf)
        ...
        Try
            ...
            If _CurrentAttach.Items.Count = 0 Or ReviewAction = ActionNames.Cancel Then
                myWS.DeleteOnBaseAttachments(System.Web.HttpContext.Current.Session("FormID"))
            End If
            ...
            finalXMLDocument.InnerXml = HttpContext.Current.Session("FormXML")
            If Not myWS.SaveXMLToFW(GetXMLValue("FormId"), finalXMLDocument, CBool(GetXMLValue("isFormNew"))) Then
                MsgBox("Failed to save the XML.  Please contact the OITS RWM Support Team.")
                UpdateXMLDocument("isSuccessfulSubmit", "false")
            Else
                If String.IsNullOrEmpty(HttpContext.Current.Session("taskID")) Then
            ...
            If GetXMLValue("isSuccessfulSubmit").ToLower = "true" Then
                ...
                myLabel.Text = "<script type='text/javascript'>" &
                   "window.allowWindowClose = true;" &
                   "console.log('allowWindowClose set to true by server-side success.');" &
                   "SubmitConfirmation('" & ConfigurationManager.AppSettings("FWWorklistURL") & "');" &
                   "</script>"
                Dim myws3 As New Maintenance.Maintenance
                myws3.ReleaseWI2(System.Web.HttpContext.Current.Session("FormID"), HttpContext.Current.Session("taskID"))
                HttpContext.Current.Session.Abandon()
            End If
            ' File.AppendAllText(myLog, "Finished DoSubmit in ASPFormhelper FormID: " & System.Web.HttpContext.Current.Session("FormID") & Now() & vbCrLf)
        Catch ex As Exception
            File.AppendAllText(myLog, "Error in DoSubmit in ASPFormhelper FormID: " & System.Web.HttpContext.Current.Session("FormID") & Now() & " " & ex.StackTrace & vbCrLf)
            ...
            If fileOK Then
                Try
                    ...
                    If IsNumeric(docid) And docid <> "0" Then
                        ...
                        UpdateXMLDocument("AttachmentCount", CInt(GetXMLValue("AttachmentCount")) + 1)
    ...
    Private Sub AddAttachmenttoSMO(ByVal docid As String, ByVal fileName As String)
        ...
        Try
            ...
            myAttach.ReferenceID = System.Web.HttpContext.Current.Session("FormID")
        ...
        Try
            ...
            If (Not myAttach Is Nothing) Then
                ...
                If result Then
                    ...
                    delAttach.ReferenceID = System.Web.HttpContext.Current.Session("FormID")
    ...
    Public Sub DeleteAllAttachments()
        ...
        result = obService.OnBaseDeleteFormAttachments(System.Web.HttpContext.Current.Session("FormID"), _KDOTFormClass.FormName)
        If Not result Then
            File.AppendAllText(myLog, "Attachments not deleted for form id: " & System.Web.HttpContext.Current.Session("FormID") & vbCrLf)
        End If
    End Sub
    ...
    Public Sub UpdateXMLDocument(ByVal NodeName As String, ByVal NodeValue As String)
        If Not NodeValue Is Nothing Then
            ...
            If NodeValue.Contains("'") Then NodeValue = Replace(NodeValue, "'", "&apos;")
            If NodeValue.Contains("""") Then NodeValue = Replace(NodeValue, """", "&quot;")
            myNav.SelectSingleNode("//" & NodeName, ns).InnerXml = NodeValue
            HttpContext.Current.Session("FormXML") = FormXML.InnerXml
        End If
    End Sub
    Public Function GetXMLValue(ByVal NodeName As String) As String
        ...
        Dim myNav As XPathNavigator = FormXML.CreateNavigator()
        Dim ns As New XmlNamespaceManager(FormXML.NameTable)
        Dim nodevalue As String = myNav.SelectSingleNode("//" & NodeName, ns).InnerXml
        If nodevalue.Contains("&amp;") Then nodevalue = Replace(nodevalue, "&amp;", "&")
        If nodevalue.Contains("&lt;") Then nodevalue = Replace(nodevalue, "&lt;", "<")
        If nodevalue.Contains("&gt;") Then nodevalue = Replace(nodevalue, "&gt;", ">")
        If nodevalue.Contains("&apos;") Then nodevalue = Replace(nodevalue, "&apos;", "'")
    ...
    Public Sub CreateHTMLFile()
        ...
        Dim xmlFileName = myXMLFiles & System.Web.HttpContext.Current.Session("FormID") & ".xml"
        xslt.Load(HttpContext.Current.Session("myPage").Server.MapPath(_KDOTFormClass.PrintViewName) & ".xsl")

        Dim myOutputFile As String = (ConfigurationManager.AppSettings("HTMLFileLocation") & System.Web.HttpContext.Current.Session("FormID") & ".html")
        If File.Exists(myOutputFile) Then File.Delete(myOutputFile)
        If File.Exists(xmlFileName) Then File.Delete(xmlFileName)

        File.AppendAllText(xmlFileName, HttpContext.Current.Session("FormXML"))
    ...
    End Sub
    Public Sub CreateHTMLFile2(myXMLFile As XmlDocument, viewName As String, seqNo As Integer)
        'This code is used to bind the xml and xslt together to create an html file for future printing
        Dim _KDOTFormClass As New KDOTFormClass
        Dim xslt As New XslCompiledTransform()
        xslt.Load(viewName & ".xsl")

        Dim myOutputFile As String = (ConfigurationManager.AppSettings("HTMLFileLocation") & System.Web.HttpContext.Current.Session("FormID") & ".html")
        If File.Exists(myOutputFile) Then File.Delete(myOutputFile)
        If File.Exists(System.Web.HttpContext.Current.Session("xmlFileName")) Then File.Delete(System.Web.HttpContext.Current.Session("xmlFileName"))

        File.AppendAllText(System.Web.HttpContext.Current.Session("xmlFileName"), myXMLFile.OuterXml)
        xslt.Transform(System.Web.HttpContext.Current.Session("xmlFileName"), myOutputFile)
        Dim myFile As String = Nothing
        myFile = File.ReadAllText(myOutputFile)
        Dim bodyLoc As Integer = InStr(myFile, "<head>")
        myFile = "<html>" & Mid(myFile, bodyLoc)
    ...
    Public Sub SaveXMLForDebug()
        ...
        File.AppendAllText(ConfigurationManager.AppSettings("XMLFileLocation") & HttpContext.Current.Session("FormID") & ".xml", HttpContext.Current.Session("FormXML"))
    End Sub
    Public Sub SaveXMLFromFile()
        'If F9 is pressed then it will update the session variable FormXML from the file
        HttpContext.Current.Session("FormXML") = File.ReadAllText(ConfigurationManager.AppSettings("XMLFileLocation") & HttpContext.Current.Session("FormID") & ".xml")
        SetPageFields()
        File.Delete(ConfigurationManager.AppSettings("XMLFileLocation") & HttpContext.Current.Session("FormID") & ".xml")
    End Sub
#Region "PDF Conversion Methods"
    ...
    Public Shared Function CreateDefaultPdfInfo(ByVal autoPersist As Boolean) As OnBaseDocumentInfo
        ...
        Dim info As OnBaseDocumentInfo = CreatePdfInfo(_KDOTFormClass.FileNetLibraryName, doc.DocumentType, String.Format("{0}.pdf", _KDOTFormClass.FormName.Replace(" ", "")), _KDOTFormClass.PrintViewName, doc.DocumentProperties, doc.SecurityEntries)
        If (autoPersist) Then
            PersistPdfInfo(info)
        End If
        Return info
    ...
    Public Shared Function CreatePdfInfo(ByVal libraryName As String, ByVal documentClass As String, ByVal documentOutputName As String, ByVal printViewName As String,
    ...
    Public Shared Sub PersistPdfInfo(ByVal registrations As IEnumerable(Of OnBaseDocumentInfo))
        ...
        xmldoc.InnerXml = HttpContext.Current.Session("FormXML")
        ...
        For Each reg As OnBaseDocumentInfo In registrations
            ...
            End Using
        Next
        HttpContext.Current.Session("FormXML") = xmldoc.InnerXml
    End Sub
#End Region
End Class
